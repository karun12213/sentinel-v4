...
const LOT_SIZE = parseFloat(process.env.LOT_SIZE) || 0.03;
const MAX_POSITIONS = parseInt(process.env.POSITIONS) || 6;
...
function log(msg, type = 'info') {
  ...
  // Push to Redis for Vercel dashboard
  try {
    const logEntry = {
      timestamp: new Date().toISOString(),
      type,
    ...
    redis.rpush('shiva:bot_logs', JSON.stringify(logEntry));
    redis.ltrim('shiva:bot_logs', -500, -1); // Keep last 500 logs
  } catch (e) {
    // Silently fail Redis push
  }
}
...
async function initSDK() {
  ...
  // Adopt existing positions on the account
  try {
    const existingPositions = await tradingAccount.getPositions();
    const symbolPositions = existingPositions.filter(p => p.symbol === SYMBOL);
    log(`Found ${symbolPositions.length} existing ${SYMBOL} position(s) on connect`, 'info');
    symbolPositions.forEach((p, i) => {
      // Normalize type: POSITION_TYPE_SELL -> SELL, POSITION_TYPE_BUY -> BUY
      const type = p.type.replace('POSITION_TYPE_', '');
      log(`  #${i+1} ${type} @ ${p.openPrice} | PnL: $${(p.profit||0).toFixed(2)} | ID: ${p.id}`, 'info');
      managedPositions.push({
        id: p.id,
        type: type,
        entry: p.openPrice,
        sl: p.stopLoss,
        highestPnl: p.profit || 0,
        beMoved: false
      });
    });
  } catch (e) {
    log(`Could not adopt positions: ${e.message}`, 'error');
  }
