...
class FVGScalpStrategy(BaseStrategy):
    ...
    """
    LOOKBACK = 40
    MAX_AGE  = 10   # looser for more signals
    ...
    def get_signal_and_wick(self, df: pd.DataFrame) -> tuple[int, float]:
        ...
        adx_min = float(os.getenv('ADX_MIN', '18'))   # lowered to 18 for more trades
        ...
        # BUY: FVG retest in uptrend, EMA20+EMA50 rising, RSI 40–72 (quality zone)
        if uptrend and 40 <= rsi <= 72 and ema20_slope > 0 and ema50_slope >= 0 and close >= bar_mid:
        ...
        # SELL: bearish FVG retest, downtrend, EMA20+EMA50 both falling, RSI 28–60 (quality zone)
        if downtrend and 28 <= rsi <= 60 and ema20_slope < 0 and ema50_slope <= 0 and close <= bar_mid:
            for fvg in reversed(fvgs):
...
class EMABounceStrategy(BaseStrategy):
    ...
    def get_signal_and_wick(self, df: pd.DataFrame) -> tuple[int, float]:
        ...
        # BUY: EMA20 bounce in uptrend, EMA20+EMA50 rising, RSI 30–70
        if (uptrend and
                30 < rsi < 70 and
                ema20_slope > 0 and
                ema50_slope >= 0 and
        ...
        # SELL: EMA20 rejection in downtrend, EMA20+EMA50 both falling, RSI 30–70
        if (downtrend and
                30 < rsi < 70 and
                ema20_slope < 0 and
                ema50_slope <= 0 and
